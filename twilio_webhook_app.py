...
@app.route('/populate-league-17', methods=['POST'])
def populate_league_17():
    """Temporary: Populate league 17 with historical season data for Slack review"""
    LEAGUE_ID = 17
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        # Get players
        cursor.execute("SELECT id, name FROM players WHERE league_id = %s AND active = TRUE ORDER BY id", (LEAGUE_ID,))
        players = cursor.fetchall()
        if not players:
            cursor.close(); conn.close()
            return jsonify({'error': 'No players found in league 17'}), 400

        player_map = {name: pid for pid, name in players}
        p = [name for _, name in players]
        logging.info(f"Populate league 17: found players {p}")

        if len(p) < 2:
            cursor.close(); conn.close()
            return jsonify({'error': f'Need at least 2 players, found {len(p)}'}), 400

        # Clean existing season data
        ...
        from league_data_adapter import calculate_wordle_number, get_week_start_date
        current_wordle = calculate_wordle_number(get_week_start_date())

        # Build 3 completed seasons + current season 4 in progress
        # Spread winners across players for realism
        # Each tuple: (week_offset_from_current * 7, winner_index, score)
        season_plans = {
            1: {
                'weeks': [(-25,0,18),(-24,1,16),(-23,2,19),(-22,0,15),(-21,3,17),(-20,1,14),(-19,0,16),(-18,0,13)],
                'champion_idx': 0, 'champion_wins': 4,
            },
            2: {
                'weeks': [(-17,1,15),(-16,4%len(p),17),(-15,1,16),(-14,2,14),(-13,1,13),(-12,3,18),(-11,5%len(p),16),(-10,1,15)],
                'champion_idx': 1, 'champion_wins': 4,
            },
            3: {
                'weeks': [(-9,2,16),(-8,0,14),(-7,3,17),(-6,2,15),(-5,4%len(p),18),(-4,2,13),(-3,2,14)],
                'champion_idx': 2, 'champion_wins': 4,
            },
        }

        inserted_weekly = 0
        for season_num, plan in season_plans.items():
            week_wordles = []
            for week_off, pidx, score in plan['weeks']:
                ww = current_wordle + (week_off * 7)
                week_wordles.append(ww)
                winner_name = p[pidx]
                pid = player_map[winner_name]
                cursor.execute("""
                    INSERT INTO weekly_winners (league_id, week_wordle_number, player_id, player_name, score)
                    VALUES (%s, %s, %s, %s, %s)
                    ON CONFLICT (league_id, week_wordle_number, player_id) DO NOTHING
                """, (LEAGUE_ID, ww, pid, winner_name, score))
                inserted_weekly += 1
            ...
            """, (LEAGUE_ID, season_num, min(week_wordles), max(week_wordles)))

            # Season winner
            champ_name = p[plan['champion_idx']]
            champ_pid = player_map[champ_name]
            cursor.execute("""
                INSERT INTO season_winners (player_id, league_id, season_number, wins)
                VALUES (%s, %s, %s, %s)
            """, (champ_pid, LEAGUE_ID, season_num, plan['champion_wins']))

        # Current season 4 — in progress
        current_season_num = 4
        ...
        cs_start = None
        for week_off, pidx, score in current_season_weeks:
            ww = current_wordle + (week_off * 7)
            if cs_start is None:
                cs_start = ww
            winner_name = p[pidx]
        ...
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, NULL)
            ON CONFLICT (league_id, season_number) DO NOTHING
        """, (LEAGUE_ID, current_season_num, cs_start))
        ...
        result = {
            ...
            'weekly_winners_inserted': inserted_weekly,
            'season_winners': {
                1: p[season_plans[1]['champion_idx']],
                2: p[season_plans[2]['champion_idx']],
                3: p[season_plans[3]['champion_idx']],
            },
            'current_season_start_week': cs_start,
        }
        logging.info(f"Populated league 17: {result}")
        return jsonify(result)

    except Exception as e:
        logging.error(f"Error populating league 17: {e}")
        import traceback
        logging.error(traceback.format_exc())
        return jsonify({'error': str(e)}), 500
